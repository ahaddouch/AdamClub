...
namespace AdamClub {
	...
	/// <summary>
	/// Summary for AfficherEquipe
	/// </summary>
	public ref class AfficherEquipe : public System::Windows::Forms::Form
	{
	public:
		AfficherEquipe(void)
		{
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->dataGridView1->CellContentClick += gcnew System::Windows::Forms::DataGridViewCellEventHandler(this, &AfficherEquipe::dataGridView1_CellContentClick);
			// 
			// bindingSource1
			// 
			this->bindingSource1->CurrentChanged += gcnew System::EventHandler(this, &AfficherEquipe::bindingSource1_CurrentChanged);
			// 
			// txt_find
			// 
			...
			this->txt_find->TextChanged += gcnew System::EventHandler(this, &AfficherEquipe::txt_find_TextChanged);
			// 
			// AfficherEquipe
			// 
			...
			this->Name = L"AfficherEquipe";
			this->Text = L"AfficherEquipe";
			this->Load += gcnew System::EventHandler(this, &AfficherEquipe::AfficherEquipe_Load);
...
#pragma endregion
	private: System::Void txt_find_TextChanged(System::Object^ sender, System::EventArgs^ e) {
		...
		try {
			...
			command->CommandText = "SELECT * FROM equipe where nom like '%" + txt_find->Text->ToString() + "%'";

			SqlDataAdapter^ dataAdapter1 = gcnew SqlDataAdapter(command);

			DataSet^ dataSet = gcnew DataSet();

			dataAdapter1->Fill(dataSet, "equipe");

			BindingSource^ bindingSource1 = gcnew BindingSource();
			bindingSource1->DataSource = dataSet->Tables["equipe"];
	...
	private: System::Void AfficherEquipe_Load(System::Object^ sender, System::EventArgs^ e) {
		...
		try {
			...
			command->CommandText = "SELECT * FROM equipe";

			SqlDataAdapter^ dataAdapter = gcnew SqlDataAdapter(command);

			DataSet^ dataSet = gcnew DataSet();

			dataAdapter->Fill(dataSet, "equipe");

			BindingSource^ bindingSource = gcnew BindingSource();
			bindingSource->DataSource = dataSet->Tables["equipe"];
